  ...
  .superRefine((value, ctx) => {
    ...
    if (duration <= 0)
      ctx.addIssue({
        code: "custom",
        path: ["endsAt"],
        message: "La fine deve seguire l’inizio.",
      });
    if (duration > 24 * 60 * 60 * 1000)
      ctx.addIssue({
        code: "custom",
        path: ["endsAt"],
        message: "Un blocco può durare al massimo 24 ore.",
      });
    if (
      ...
      ctx.addIssue({
        ...
        message: "Scegli almeno un giorno.",
      });
  });
export const timeBlockPatchSchema = z
  .object({
    ...
    status: z.enum(TIME_BLOCK_STATUSES).optional(),
    itemId: idSchema.nullable().optional(),
    recurrence: recurrenceSchema,
  })
  .strict();
export const plannerCategorySchema = z
  .object({
    name: z
      .string()
      .trim()
      .min(1, "Inserisci un nome per la categoria.")
      .max(60),
    color: z
      .string()
      .trim()
      .regex(/^#[0-9a-fA-F]{6}$/, "Inserisci un colore HEX valido."),
    icon: z.string().trim().min(1).max(40).nullable().optional(),
  })
  .strict();
export const plannerCategoryPatchSchema = z
  .object({
    name: z
      .string()
      .trim()
  ...
  .strict();
export const plannerRangeSchema = z
  .object({ from: datetimeSchema, to: datetimeSchema })
  .strict()
  .superRefine((v, ctx) => {
    if (new Date(v.to).getTime() <= new Date(v.from).getTime())
      ctx.addIssue({ code: "custom", message: "Intervallo non valido." });
    if (
      ...
      ctx.addIssue({
        code: "custom",
        message: "Scegli al massimo una settimana.",
      });
  });
