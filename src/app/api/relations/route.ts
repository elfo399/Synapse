...
export const GET = (request: Request) =>
  route(async () => {
    ...
    const itemId = idSchema
      .optional()
      .parse(new URL(request.url).searchParams.get("itemId") ?? undefined);
...
export const POST = (request: Request) =>
  route(async () => {
    ...
    return json(
      {
        relation: await createRelation(
          user.id,
          relationSchema.parse(await readJson(request)),
        ),
      },
      201,
    );
