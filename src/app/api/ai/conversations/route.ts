...
const createSchema = z.object({ mode: z.enum(["KNOWLEDGE", "GENERAL"]).default("KNOWLEDGE"), title: z.string().trim().max(160).optional() }).strict();
...
export const POST = (request: Request) => route(async () => {
  ...
  return json({ conversation: await createConversation(user.id, input.mode, input.title) }, 201);
});
