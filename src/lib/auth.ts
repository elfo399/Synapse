...
if (process.env.NODE_ENV === "production" && process.env.NEXT_PHASE !== "phase-production-build") {
  ...
  if (!process.env.BETTER_AUTH_URL) throw new Error("BETTER_AUTH_URL must be configured.");
...
export const auth = betterAuth({
  ...
  advanced: { useSecureCookies: new URL(baseURL).protocol === "https:", cookiePrefix: "secondbrain" },
  rateLimit: { enabled: true, storage: "database", window: 60, max: 100, customRules: { "/sign-in/email": { window: 60, max: 10 } } },
