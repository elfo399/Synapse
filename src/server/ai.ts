...
function isQwen3Model(model: string) {
...
export async function ollama(path: string, init?: RequestInit) {
  ...
  if (!config.enabled)
    throw new HttpError(503, "L'assistente AI non è abilitato.");
  if (!config.validUrl)
    throw new HttpError(503, "La configurazione di Ollama non è valida.");
...
export async function aiStatus() {
  ...
  if (!config.enabled || !config.validUrl)
    return {
      state: config.enabled ? ("offline" as const) : ("disabled" as const),
      ...base,
  ...
  try {
    ...
    return {
      state: chatModelReady ? ("ready" as const) : ("model_missing" as const),
      ...base,
      ollamaReachable: true,
      chatModelReady,
      embeddingModelReady: has(config.embeddingModel),
    };
  } catch {
    return {
...
function toolPlanPrompt(args: {
  ...
  const conversation = args.history
    ...
    .map(
      (message) =>
        `${message.role === "user" ? "Utente" : "Synapse"}: ${message.content.slice(0, 600)}`,
    )
    .join("\n");
...
export function parseToolPlan(
  ...
  | { ok: false; reason: "invalid_json" | "invalid_tool" } {
  const candidate = content.match(/\{[\s\S]*\}/)?.[0] || content;
  let parsed: { tools?: unknown };
  try {
    parsed = JSON.parse(candidate) as { tools?: unknown };
  } catch {
  ...
  if (
    ...
    !parsed.tools.length ||
    parsed.tools.length > MAX_TOOL_CALLS
  )
  ...
  for (const raw of parsed.tools) {
    ...
    if (typeof call.name !== "string" || !TOOL_NAMES.has(call.name as ToolName))
      return { ok: false, reason: "invalid_tool" };
    const name = call.name as ToolName;
    const query =
      typeof call.query === "string"
        ? call.query.trim().slice(0, 500)
        : undefined;
    const project =
      typeof call.project === "string"
        ? call.project.trim().slice(0, 180)
        : undefined;
    const itemType =
      typeof call.itemType === "string" ? call.itemType : undefined;
    const timezone =
      typeof call.timezone === "string" ? call.timezone.trim() : undefined;
    if (name === "list_items" && !validItemType(itemType))
      return { ok: false, reason: "invalid_tool" };
    if (name === "search_knowledge" && !query)
      return { ok: false, reason: "invalid_tool" };
    if ((name === "get_project" || name === "get_relations") && !project)
      return { ok: false, reason: "invalid_tool" };
    if (name === "get_time" && !isValidTimezone(timezone))
      return { ok: false, reason: "invalid_tool" };
    tools.push({
      name,
      query,
...
async function requestToolPlan(args: {
  question: string;
  webAllowed: boolean;
  activeProjectTitle?: string | null;
  history: ChatMessage[];
...
async function planTools(args: {
  ...
  try {
    ...
    if (first.ok) {
      traceAi("planner_requested", {
        tools: first.plan.tools.map((tool) => tool.name).join(","),
        toolCount: first.plan.tools.length,
        elapsedMs: Date.now() - startedAt,
      });
      return { state: "planned", plan: first.plan, corrected: false };
    }
    traceAi("planner_parse_error", { reason: first.reason });
    const corrected = await requestToolPlan({
      ...args,
      correction: first.reason,
    });
    if (corrected.ok) {
      traceAi("planner_requested", {
        tools: corrected.plan.tools.map((tool) => tool.name).join(","),
        toolCount: corrected.plan.tools.length,
        corrected: true,
        elapsedMs: Date.now() - startedAt,
      });
      return { state: "planned", plan: corrected.plan, corrected: true };
    }
    traceAi("planner_parse_error", {
...
async function resolveProject(
  ...
  const projects = await prisma.item.findMany({
    where: { userId, type: "PROJECT", archivedAt: null, deletedAt: null },
    select: {
      id: true,
      title: true,
      status: true,
...
function itemSource(item: {
  ...
  return {
    ...
    title: item.title,
    excerpt:
      item.content?.replace(/\s+/g, " ").slice(0, 280) ||
      `${item.type} senza descrizione.`,
    href: getItemHref(item),
  };
}

export async function listAssistantItems(
  userId: string,
  itemType: unknown,
  limit: unknown,
): Promise<AssistantCatalogResult> {
  ...
  const where = { userId, type, archivedAt: null, deletedAt: null } as const;
...
async function projectContext(
      ...
      OR: [
        {
          ...
          source: { userId, deletedAt: null },
          target: { deletedAt: null },
        },
        {
          targetItemId: project.id,
          target: { userId, deletedAt: null },
  ...
  const linked = related
    .map((relation) =>
      relation.sourceItemId === project.id ? relation.target : relation.source,
    )
    .filter((item) => item.id !== project.id);
  const unique = linked.filter(
    (item, index) =>
      linked.findIndex((candidate) => candidate.id === item.id) === index,
  );
  return {
    sources: uniqueSources([
      itemSource({ ...project, type: "PROJECT" }),
      ...unique.map(itemSource),
    ]),
    context: [
      `Progetto: ${project.title}${project.status ? ` — stato ${project.status}` : ""}.`,
      project.content
        ? `Descrizione: ${project.content.slice(0, 1800)}`
...
async function searchKnowledge(
  ...
  if (lexical.status === "rejected" && semantic.status === "rejected") {
    ...
    return {
      ...
      sources: [],
      context:
        "Il recupero delle conoscenze non ? disponibile per questa risposta; l'assenza di fonti non indica che l'archivio sia vuoto.",
    };
  }
  const lexicalIds =
    lexical.status === "fulfilled"
      ? lexical.value.items.map((item) => item.id)
      : [];
  const semanticIds =
    semantic.status === "fulfilled"
      ? semantic.value.map((item) => item.itemId)
      : [];
  ...
  if (!ids.length) {
    ...
    return {
      ...
      sources: [],
      context:
        "Nessuna conoscenza pertinente ? stata trovata per questa ricerca.",
    };
  }
  const items = await prisma.item.findMany({
    where: { userId, id: { in: ids }, archivedAt: null, deletedAt: null },
  ...
  traceAi("tool_result", {
    ...
    ftsAvailable: lexical.status === "fulfilled",
    vectorAvailable: semantic.status === "fulfilled",
  });
  return {
    state: "ok",
    sources,
...
function applyConversationMode(
  ...
  if (mode === "WEB")
    return {
      tools: [
        webAllowed
          ? { name: "search_web", query: question }
          : { name: "respond" },
      ],
    };
  if (mode === "COMBINED") {
    const withLocal = plan.tools.some((tool) =>
      [
        "search_knowledge",
        "list_items",
        "get_project",
      ...
      ? plan.tools
      : [
          { name: "search_knowledge" as const, query: question, limit: 8 },
          ...plan.tools,
        ];
    const withWeb =
      webAllowed && !withLocal.some((tool) => tool.name === "search_web")
  ...
  for (const tool of args.plan.tools) {
    ...
    if (tool.name === "search_knowledge") {
      const knowledge = await searchKnowledge(
        args.userId,
        tool.query!,
        clampLimit(tool.limit),
      );
      result.sources.push(...knowledge.sources);
      result.context.push(
        `Risultato search_knowledge (${knowledge.state}):\n${knowledge.context}`,
      );
      continue;
    }
    if (tool.name === "list_items") {
      const catalog = await listAssistantItems(
        args.userId,
        tool.itemType,
        tool.limit,
      );
      if (catalog.state === "invalid_type") {
        result.context.push(
          "Lo strumento list_items ha ricevuto un tipo non valido: non ? stata eseguita una ricerca generica e non si pu? dedurre che l'archivio sia vuoto.",
        );
      } else {
        result.sources.push(...catalog.sources);
        result.context.push(
          `Catalogo strutturato Synapse:\n${JSON.stringify({ itemType: catalog.itemType, total: catalog.total, items: catalog.items, truncated: catalog.truncated })}`,
        );
      }
      continue;
    }
    if (tool.name === "get_project" || tool.name === "get_relations") {
      ...
      if (!project) {
        ...
        result.context.push(
          "Il progetto richiesto non ? stato identificato in modo univoco nell'account corrente. Questo non equivale a un archivio di progetti vuoto.",
        );
        continue;
      }
      ...
      traceAi("tool_result", {
        tool: tool.name,
        state: "ok",
        returned: context.sources.length,
      });
      result.context.push(
        `${tool.name === "get_relations" ? "Relazioni del progetto" : "Contesto del progetto"}:\n${context.context}`,
      );
      continue;
    }
    if (tool.name === "search_web") {
      if (!args.webAllowed) {
        result.context.push(
          "La ricerca Web non ? autorizzata per questa richiesta.",
        );
        continue;
      }
      ...
      try {
        ...
        result.context.push(
          "La ricerca Web non ? disponibile per questa risposta: non presentare informazioni come aggiornate o verificate.",
        );
      }
      continue;
    }
    if (tool.name === "get_time")
      result.context.push(timeContext(tool.timezone!));
  }

  const sources = uniqueSources(result.sources);
  traceAi("final_context", {
    sourceCount: sources.length,
    toolCount: result.used.size,
  });
...
function strategyFromTools(tools: Set<ToolName>): AiStrategy {
  const local =
    tools.has("search_knowledge") ||
    tools.has("list_items") ||
    tools.has("get_project") ||
    tools.has("get_relations");
  ...
  try {
    ...
    const activeProject = conversation.activeProjectId
      ? await prisma.item.findFirst({
          where: {
            id: conversation.activeProjectId,
            userId: args.userId,
            type: "PROJECT",
            archivedAt: null,
            deletedAt: null,
    ...
    const webAllowed = Boolean(
      options.webSearch &&
      webSearchConfig().enabled &&
      webSearchConfig().validUrl,
    );
    const planning = await planTools({
      question: content,
      webAllowed,
    ...
    if (planning.state === "failed") {
      execution.context.unshift(
        `Il pianificatore degli strumenti non ? riuscito a completare il recupero (${planning.reason}). Non dichiarare l'archivio vuoto: spiega che le fonti personali non sono state consultate per questa risposta.`,
      );
    ...
    if (conversation.title === "Nuova conversazione") {
      await prisma.aiConversation.updateMany({
        ...
        data: { title: titleFromFirstMessage(content) },
      });
    ...
    if (execution.activeProjectId !== conversation.activeProjectId) {
      await prisma.aiConversation.update({
        ...
        data: { activeProjectId: execution.activeProjectId ?? null },
      });
  ...
  const response = await ollama("/api/chat", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: buildSystemPrompt(strategy, sources, toolContext),
        },
        ...history,
...
export async function completeGeneration(args: {
  ...
  const cited = new Set(
    [...args.content.matchAll(/\[S\d+\]/g)].map((match) => match[0]),
  );
  const persistedSources = args.sources.map((source) => ({
    ...source,
    usage:
      source.citation && cited.has(source.citation)
        ? ("CITED" as const)
        : ("CONSULTED" as const),
  }));
...
export async function beginGeneration(
  args: Parameters<typeof beginAutomaticGeneration>[0],
) {
...
export async function openOllamaStream(args: {
  ...
  const manualStrategy: AiStrategy =
    args.mode === "WEB"
      ? "WEB"
      : args.mode === "COMBINED"
        ? "COMBINED"
...
export async function createConversation(
  ...
  mode: AiMode = "AUTO",
  title = "Nuova conversazione",
  preferences: AiChatOptions = {},
) {
  return prisma.aiConversation.create({
    data: {
      userId,
      mode,
      title: title.trim().slice(0, 160) || "Nuova conversazione",
      webSearchEnabled: preferences.webSearch ?? mode === "AUTO",
      reasoningEnabled: preferences.reasoning ?? false,
    },
  });
}
...
export async function updateConversationPreferences(
  ...
  const updated = await prisma.aiConversation.updateMany({
    where: { id, userId },
    data: {
      webSearchEnabled: Boolean(preferences.webSearch),
      reasoningEnabled: Boolean(preferences.reasoning),
    },
  });
  if (!updated.count) throw new HttpError(404, "Conversazione non trovata.");
}
