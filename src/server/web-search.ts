...
function isOfficial(domain: string) { return /(^|\.)(nodejs\.org|typescriptlang\.org|react\.dev|nextjs\.org|developer\.mozilla\.org|docs\.docker\.com|kubernetes\.io|python\.org|github\.com)$/i.test(domain); }
export async function searchWebAutomatically(question: string): Promise<WebResult[]> {
  const candidates = await searchWeb(question); const unique = new Map<string, WebResult>();
  for (const candidate of candidates) { const key = new URL(candidate.url).hostname.replace(/^www\./, "") + new URL(candidate.url).pathname.replace(/\/$/, ""); if (!unique.has(key)) unique.set(key, candidate); }
  const technical = /\b(versione|node|typescript|javascript|docker|api|errore|programmazione|software|documentazione)\b/i.test(question);
  return [...unique.values()].sort((a, b) => Number(technical && isOfficial(b.domain)) - Number(technical && isOfficial(a.domain))).slice(0, webSearchConfig().maxResults);
}
