...
function plainSearchSnippet(snippet: string): string {
  return snippet
    .replace(
      /\[\[([^\]\n]+)\]\]/g,
      (_, target: string) => target.split("|").at(-1)?.trim() ?? target,
    )
    .replace(/\[\[|\]\]/g, "")
    .replace(/!?\[([^\]\n]+)\]\([^)]*\)/g, "$1")
    .replace(/(^|\n)\s{0,3}(?:#{1,6}\s+|>\s?|[-*+]\s+|\d+[.)]\s+)/g, "$1")
    .replace(/(`{1,3}|\*{1,2}|_{1,2}|~~)(.*?)\1/g, "$2")
    .replace(/\s+/g, " ")
    .trim();
}
...
export async function searchItems(userId: string, input: unknown) {
  ...
  if (query.archive === "archived")
    filters.push(Prisma.sql`i."archivedAt" IS NOT NULL`);
  if (query.tag)
    filters.push(
      Prisma.sql`EXISTS (SELECT 1 FROM "ItemTag" it JOIN "Tag" t ON t.id = it."tagId" WHERE it."itemId" = i.id AND it."userId" = ${userId} AND t."normalizedName" = ${query.tag.normalize("NFKC").trim().toLowerCase()})`,
    );
  ...
  const [hits, count] = await prisma.$transaction(
    [
      ...
      prisma.$queryRaw<{ total: bigint }[]>(
        Prisma.sql`SELECT count(*) AS total FROM "Item" i WHERE ${where}`,
      ),
    ],
