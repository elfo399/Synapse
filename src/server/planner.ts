...
import {
  categorySummary,
  ensurePlannerCategories,
  getPlannerCategories,
  ownedPlannerCategory,
} from "./planner-categories";
...
function serialized(block: {
  ...
  const actualMinutes = (block.sessions ?? []).reduce(
    (total, session) =>
      total +
      Math.max(
        0,
  ...
  return {
    ...
    category: categorySummary(block.plannerCategory),
    status: block.status,
    itemId: block.itemId,
    recurrence: block.recurrence as Recurrence | null,
    item: block.item
      ? { ...block.item, dueAt: block.item.dueAt?.toISOString() ?? null }
      : null,
    actualMinutes,
    conflict: false,
  };
...
function recurrenceMatches(block: TimeBlockSummary, occurrence: Date): boolean {
  ...
  if (rule.until && occurrence.getTime() > new Date(rule.until).getTime())
    return false;
  if (rule.frequency === "DAILY") return true;
  return (
    rule.weekdays?.length
      ? rule.weekdays
      : [new Date(block.startsAt).getUTCDay()]
  ).includes(occurrence.getUTCDay());
}
...
function expandRecurring(
  ...
  if (!block.recurrence)
    return block.startsAt < to.toISOString() &&
      block.endsAt > from.toISOString()
      ? [block]
      : [];
  const sourceStart = new Date(block.startsAt);
  const sourceEnd = new Date(block.endsAt);
  const duration = sourceEnd.getTime() - sourceStart.getTime();
  const output: TimeBlockSummary[] = [];
  const cursor = new Date(
    Date.UTC(from.getUTCFullYear(), from.getUTCMonth(), from.getUTCDate()),
  );
  const sourceTime: [number, number, number] = [
  ...
  for (let i = 0; i < 10 && cursor < to; i += 1) {
    ...
    if (
      cursor >= sourceStart &&
      recurrenceMatches(block, cursor) &&
      cursor < to
    )
      output.push({
        ...block,
        id: `${block.id}:${cursor.toISOString().slice(0, 10)}`,
...
function markConflicts(blocks: TimeBlockSummary[]): TimeBlockSummary[] {
  return blocks.map((block, index) => ({
    ...
    conflict: blocks.some(
      (other, otherIndex) =>
        otherIndex !== index &&
        new Date(block.startsAt) < new Date(other.endsAt) &&
        new Date(block.endsAt) > new Date(other.startsAt),
    ),
  }));
}
...
export async function getPlanner(
  ...
  const range = plannerRangeSchema.parse(rawRange);
  ...
  const [stored, tasks, templates, categories] = await prisma.$transaction([
    prisma.timeBlock.findMany({
      ...
      include: {
        plannerCategory: { select: categorySelect },
        item: { where: { deletedAt: null }, select: itemSelect },
        sessions: {
          where: {
            startedAt: { lt: to },
    ...
    prisma.plannerCategory.findMany({
      where: { userId },
      orderBy: [{ archivedAt: "asc" }, { sortOrder: "asc" }, { name: "asc" }],
      select: categorySelect,
    }),
  ]);
  const blocks = markConflicts(
    stored
      .flatMap((block) => expandRecurring(serialized(block), from, to))
  ...
  const values = new Map(summary.map((value) => [value.category.id, value]));
  for (const block of blocks) {
    const value = values.get(block.categoryId);
    if (value) {
      value.planned += Math.round(
        (new Date(block.endsAt).getTime() -
          new Date(block.startsAt).getTime()) /
          60_000,
      );
      value.actual += block.actualMinutes;
    }
  }
  return {
    blocks,
...
export async function createTimeBlock(userId: string, raw: unknown) {
  ...
  return withUserTransaction(userId, async (tx) => {
    await ensurePlannerCategories(tx, userId);
    await ownedItem(tx, userId, input.itemId);
    const category = await ownedPlannerCategory(tx, userId, input.categoryId);
    const block = await tx.timeBlock.create({
      data: {
        ...
        status: input.status,
        recurrence: input.recurrence ?? Prisma.JsonNull,
...
export async function updateTimeBlock(
  ...
  return withUserTransaction(userId, async (tx) => {
    const previous = await tx.timeBlock.findFirst({ where: { userId, id } });
    if (!previous) throw new HttpError(404, "Blocco non trovato.");
    const startsAt = input.startsAt
      ? new Date(input.startsAt)
      : previous.startsAt;
    const endsAt = input.endsAt ? new Date(input.endsAt) : previous.endsAt;
    if (
      endsAt <= startsAt ||
      endsAt.getTime() - startsAt.getTime() > 86_400_000
    )
      throw new HttpError(400, "Controlla l’orario del blocco.");
    await ownedItem(tx, userId, input.itemId);
    const category = input.categoryId
      ? await ownedPlannerCategory(tx, userId, input.categoryId)
      : null;
    const recurrence =
      input.recurrence === undefined
        ? undefined
        : (input.recurrence ?? Prisma.JsonNull);
...
const storedTemplateBlockSchema = z
  .object({
    ...
    itemId: z.string().min(1).max(128).nullable().optional(),
  })
  .strict();
export async function applyTemplate(userId: string, id: string, raw: unknown) {
  ...
  return withUserTransaction(userId, async (tx) => {
    await ensurePlannerCategories(tx, userId);
    const template = await tx.plannerTemplate.findFirst({
      where: { userId, id },
      select: { blocks: true },
    });
    if (!template) throw new HttpError(404, "Template non trovato.");
    const rows = z
      .array(storedTemplateBlockSchema)
      .min(1)
      .max(30)
      .parse(template.blocks);
    const categories = await getPlannerCategories(tx, userId);
    const created = [];
    for (const row of rows) {
      const originalStart = new Date(row.startsAt);
      const originalEnd = new Date(row.endsAt);
      const duration = originalEnd.getTime() - originalStart.getTime();
      if (duration <= 0 || duration > 86_400_000) continue;
      const category = row.categoryId
        ? await ownedPlannerCategory(tx, userId, row.categoryId)
        : categories.find(
            (value) =>
              value.name ===
              defaultPlannerCategory(row.category ?? "OTHER").name,
          );
      if (!category)
        throw new HttpError(
          400,
          "Il template contiene una categoria non disponibile.",
        );
      await ownedItem(tx, userId, row.itemId);
      const timezone = row.timezone ?? "Europe/Rome";
      const time = timeParts(originalStart, timezone);
      const startsAt = zonedDateTimeToUtc(input.date, time.hour, time.minute);
      const block = await tx.timeBlock.create({
        data: {
          userId,
          title: row.title,
...
export async function saveTemplate(userId: string, raw: unknown) {
  const input = z
    .object({
      ...
      blocks: z.array(timeBlockSchema).min(1).max(30),
    })
    .strict()
    .parse(raw);
  return withUserTransaction(userId, async (tx) => {
    ...
    for (const block of input.blocks)
      await ownedPlannerCategory(tx, userId, block.categoryId);
    const blocks = JSON.parse(
      JSON.stringify(input.blocks),
    ) as Prisma.InputJsonValue;
    return tx.plannerTemplate.upsert({
      where: { userId_name: { userId, name: input.name } },
      create: { userId, name: input.name, blocks },
      update: { blocks },
      select: { id: true, name: true, blocks: true },
    });
  });
