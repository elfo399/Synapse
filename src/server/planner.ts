import { Prisma, TimeBlockStatus } from "@prisma/client";
...
function expandRecurring(block: TimeBlockSummary, from: Date, to: Date): TimeBlockSummary[] {
  ...
  const sourceTime: [number, number, number] = [sourceStart.getUTCHours(), sourceStart.getUTCMinutes(), sourceStart.getUTCSeconds()];
...
export async function getPlanner(userId: string, rawRange: unknown): Promise<PlannerData> {
  ...
  const [stored, tasks, templates] = await prisma.$transaction([
    prisma.timeBlock.findMany({ where: { userId, OR: [{ startsAt: { lt: to }, endsAt: { gt: from } }, { recurrence: { not: Prisma.DbNull } }] }, orderBy: { startsAt: "asc" }, include: { item: { select: itemSelect }, sessions: { where: { startedAt: { lt: to }, OR: [{ endedAt: null }, { endedAt: { gt: from } }] }, select: { startedAt: true, endedAt: true } } } }),
...
export async function createTimeBlock(userId: string, raw: unknown) {
  ...
  return withUserTransaction(userId, async (tx) => { await ownedItem(tx, userId, input.itemId); const block = await tx.timeBlock.create({ data: { userId, title: input.title, description: input.description, startsAt: new Date(input.startsAt), endsAt: new Date(input.endsAt), timezone: input.timezone, category: input.category, status: input.status, recurrence: input.recurrence ?? Prisma.JsonNull, itemId: input.itemId ?? null }, include: { item: { select: itemSelect }, sessions: { select: { startedAt: true, endedAt: true } } } }); return serialized(block); });
}
...
export async function updateTimeBlock(userId: string, id: string, raw: unknown) {
  ...
  return withUserTransaction(userId, async (tx) => {
    ...
    const recurrence = input.recurrence === undefined ? undefined : input.recurrence ?? Prisma.JsonNull;
    const block = await tx.timeBlock.update({ where: { id }, data: { title: input.title, description: input.description, startsAt, endsAt, timezone: input.timezone, category: input.category, status: input.status, recurrence, itemId: input.itemId }, include: { item: { select: itemSelect }, sessions: { select: { startedAt: true, endedAt: true } } } }); return serialized(block);
  });
}
...
export async function saveTemplate(userId: string, raw: unknown) { const input = z.object({ name: z.string().trim().min(1).max(100), blocks: z.array(timeBlockSchema).min(1).max(30) }).strict().parse(raw); const blocks = JSON.parse(JSON.stringify(input.blocks)) as Prisma.InputJsonValue; return prisma.plannerTemplate.upsert({ where: { userId_name: { userId, name: input.name } }, create: { userId, name: input.name, blocks }, update: { blocks }, select: { id: true, name: true, blocks: true } }); }
