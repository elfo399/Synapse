...
export async function syncWikiLinks(
  ...
  if (links.length)
    await tx.wikiReference.createMany({
      data: links.map((link) => ({
        userId,
        sourceItemId,
        title: link.title,
        titleNormalized: link.normalized,
      })),
    });
...
export async function resolveNewTitle(
  ...
  const waiting = await tx.wikiReference.findMany({
    where: {
      userId,
      titleNormalized: normalized,
      sourceItemId: { not: targetItemId },
    },
...
export async function propagateTitleRename(
  ...
  const references = await tx.wikiReference.findMany({
    where: { userId, titleNormalized: oldNormalized },
    include: { source: { select: { id: true, content: true } } },
  });
