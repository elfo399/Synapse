          ...
          onCloseAutoFocus={(event) => {
            ...
            if (
              ...
              if (
                ...
                previousFocus.current !== document.documentElement &&
                previousFocus.current.getClientRects().length > 0
              )
...
export function Loading({
  label = "Caricamento dello spazio personale…",
}: {
  label?: string;
}) {
  return (
...
export function ItemRow({
  ...
  return (
    <div className="item-row">
      ...
      <span className="item-row-date">{dateLabel(item.updatedAt)}</span>
      {action || (
        <ArrowUpRight size={15} className="muted" aria-hidden="true" />
      )}
    </div>
  );
}
