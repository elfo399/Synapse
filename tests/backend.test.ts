...
describe("wikilinks", () => {
  it("deduplicates target identity and supports a display alias", () => {
    const links = extractWikiLinks(
      "[[Docker]] and [[docker|containers]] and [[ PostgreSQL ]] and [[missing]]",
    );
    expect(links.map((link) => link.normalized)).toEqual([
      "docker",
      "postgresql",
      "missing",
    ]);
  ...
  it("ignores escaped links and fenced/inline code", () => {
    const content =
      "\\[[Escaped]] `[[Inline]]`\n```md\n[[Fenced]]\n```\n~~~\n[[Also code]]\n~~~\n[[Real]]";
    expect(extractWikiLinks(content).map((link) => link.title)).toEqual([
      "Real",
    ]);
  });
...
describe("boundary validation", () => {
  ...
  it("rejects unknown ownership fields and unsafe bookmark protocols", () => {
    expect(
      itemSchema.safeParse({ title: "x", userId: "another-owner" }).success,
    ).toBe(false);
    expect(
      itemSchema.safeParse({ title: "x", url: "javascript:alert(1)" }).success,
    ).toBe(false);
    expect(
      itemSchema.safeParse({ title: "x", url: "https://example.com" }).success,
    ).toBe(true);
  });
  it("bounds pagination and content", () => {
    ...
    expect(
      itemSchema.safeParse({ title: "x", content: "a".repeat(500_001) })
        .success,
    ).toBe(false);
  });
