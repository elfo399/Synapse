...
describe("trusted deployment origins", () => {
  it.each(loopbackOrigins)(
    "accepts equivalent loopback addresses when configured as %s",
    (baseURL) => {
      expect(getTrustedOrigins(baseURL)).toEqual(
        expect.arrayContaining(loopbackOrigins),
      );
  ...
  ])("trusts only the configured origin for remote deployment %s", (baseURL) =>
    expect(getTrustedOrigins(`${baseURL}/auth`)).toEqual([baseURL]),
  );
});
...
describe("mutation origin protection", () => {
  ...
  it.each(loopbackOrigins)(
    "allows a same-origin mutation through %s",
    (origin) => {
  ...
  it.each([
    ...
    expect(() => assertMutationOrigin(request(origin))).toThrow(
      "Questa richiesta non proviene da Synapse.",
    );
  });

  it("still rejects cross-site requests even from a trusted origin", () => {
    expect(() =>
      assertMutationOrigin(request("http://127.0.0.1:3000", "cross-site")),
  ...
  it("does not trust loopback origins when deployed at a remote origin", () => {
    ...
    expect(() =>
      assertMutationOrigin(request("https://brain.example")),
    ).not.toThrow();
